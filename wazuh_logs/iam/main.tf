...
resource "oci_identity_policy" "wazuh_log_backups" {
  ...
  statements = [
    "Allow dynamic-group ${oci_identity_dynamic_group.rsa_access_dynamic_group.name} to manage objects in compartment id ${var.compartment_ocid} where target.bucket.name='${var.wazuh_backup_bucket_name}'",
